...
namespace WebAPI.Controllers
{
    ...
    public class WProfileVisitsPageController : ApiController
    {
        [HttpGet]
        public IHttpActionResult ReadOne(long id)
        {
            var response = new ResponseVMR<WProfileVisitsPageVMR>();

            try
            {
                response.data = WProfileVisitsPageBLL.ReadOne(id);
            }
            catch (Exception e)
            {
